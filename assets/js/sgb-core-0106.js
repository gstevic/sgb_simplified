function getData(){
    $.ajax({
      ...
      //  document.getElementById("current_avg_temp").innerHTML = current_avg_temp;
       // document.getElementById("current_avg_hum").innerHTML = current_avg_hum;
        ...
        //m2 current data set
        m1_status = parseInt(myArray[27]);
        m1_time = formatDate(myArray[28]);
        m1_command =parseInt(myArray[29]);

        //m2 current data set
        m2_status = parseInt(myArray[30]);
        m2_time = formatDate(myArray[31]);
        m2_command =parseInt(myArray[32]);
        //t3 old
        //t3_old_temp = parseFloat(myArray[25]).toFixed(1);
        //t3_old_hum = parseFloat(myArray[26]).toFixed(1);
       //temp diffrence start
       ...
       const slider1 = document.querySelector('.input-range-m1');
       const slider2 = document.querySelector('.input-range-m2');
     
       slider1.addEventListener('input', inputRangeM1);
       slider2.addEventListener('input', inputRangeM2);


      $("#status-m1").text(m1_status);
      var current_status_m1 = $('#status-m1').text();
      console.log('current_status-m1:'+current_status_m1);

      $("#status-m2").text(m2_status);
      var current_status_m2 = $('#status-m2').text();
      console.log('current_status-m2:'+current_status_m2);

      $('#m1_button').prop('disabled', true);
      $('#m2_button').prop('disabled', true);

        //   var range-m1 = $('input-range-m2')
        //   document.getElementById("new-command-m2").innerHTML = '';
   
        // var range-m2 = $('input-range-m2')
        // document.getElementById("new-command-m2").innerHTML = '';

         function inputRangeM1(){
           //var current_state_m2 = value.html(this.value);
           var new_command_M1 = this.value;
           new_command_M1 = parseInt(new_command_M1);
           current_status_m1 = parseInt(current_status_m1);
           console.log('new command M1:'+new_command_M1);
           if(current_status_m1 != new_command_M1){
               if(new_command_M1 > current_status_m1){
               document.getElementById("new-command-m1").innerHTML = 'Nova komanda: OTVORI na '+new_command_M1+'cm';
               }
               else if(new_command_M1 < current_status_m1 && new_command_M1 > 0){
                   document.getElementById("new-command-m1").innerHTML = 'Nova komanda: ZATVORI na '+new_command_M1+'cm';
               }
               else if(new_command_M1 == 0){
               document.getElementById("new-command-m1").innerHTML = 'Nova komanda: ZATVORI!';
               }
           }
           if(current_status_m1 == new_command_M1){
               document.getElementById("new-command-m1").innerHTML = 'Trenuto stanje motora: '+new_command_M1+'cm';
               $('#m1_button').prop('disabled', true);
           }
           else{
         ...
         function inputRangeM2() {
            //var current_state_m2 = value.html(this.value);
            var new_command_M2 = this.value;
            new_command_M2 = parseInt(new_command_M2);
            current_status_m2 = parseInt(current_status_m2);
            console.log('new command m2:'+new_command_M2);
            if(current_status_m2 != new_command_M2){
                if(new_command_M2 > current_status_m2){
                document.getElementById("new-command-m2").innerHTML = 'Nova komanda: OTVORI na '+new_command_M2+'cm';
                }
                else if(new_command_M2 < current_status_m2 && new_command_M2 > 0){
                    document.getElementById("new-command-m2").innerHTML = 'Nova komanda: ZATVORI na '+new_command_M2+'cm';
                }
                else if(new_command_M2 == 0){
                document.getElementById("new-command-m2").innerHTML = 'Nova komanda: ZATVORI!';
                }
            }
            if(current_status_m2 == new_command_M2){
                document.getElementById("new-command-m2").innerHTML = 'Trenuto stanje motora: '+new_command_M2+'cm';
                $('#m2_button').prop('disabled', true);
            }
            else{
                $('#m2_button').prop('disabled', false);
            }
          }
          ...
          if(m1_command != 1000){
            ...
            document.getElementById("m2_button").innerHTML = 'Otkaži komandu';
          }else{
          ...
          if(m2_command != 1000){
            ...
            $('#m2_button').removeClass('btn-block btn-outline-primary').addClass('btn-block btn-danger');
            $('#m2_button').attr('onclick', "m2Rollback()");
            document.getElementById("m2_button").innerHTML = 'Otkaži komandu';
          }else{
            $('#m2_button').removeClass('btn-block btn-danger').addClass('btn-block btn-outline-primary');
            $('#m2_button').attr('onclick', "m2Click()");
            $('#m2').prop('disabled', false);
            document.getElementById("m2_button").innerHTML = 'Izvrši komandu';
          }
   ...
   var now = new Date().getTime();
          ...
          var days_m1 = Math.floor(distance_m1 / (1000 * 60 * 60 * 24));
          var hours_m1 = Math.floor((distance_m1 % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
          var minutes_m1 = Math.floor((distance_m1 % (1000 * 60 * 60)) / (1000 * 60));
          var seconds_m1 = Math.floor((distance_m1 % (1000 * 60)) / 1000);
       ...
       //COUNTER FOR m1 BEGIN
      if(days_m1>0){
        // Output the result in an element with id="demo"
        document.getElementById("m1_time").innerHTML = days_m1 + "d " + hours_m1 + "h "+ minutes_m1 + "min " + seconds_m1 + "sek ";
        $("#m1_temp").addClass("temp_color_wait");
        $("#m1_hum").addClass("hum_color_wait");
    }
    else if(hours_m1>0){
        document.getElementById("m1_time").innerHTML = hours_m1 + "h "+ minutes_m1 + "min " + seconds_m1 + "sek ";
        $("#m1_temp").addClass("temp_color_wait");
        $("#m1_hum").addClass("hum_color_wait");
    }
    else{
    // Output the result in an element with id="demo"
    if(minutes_m1<1){
        document.getElementById("m1_time").innerHTML = " Motor se javio prije "+minutes_m1+" min " +seconds_m1+" sek";
        $("#m1_temp").removeClass("temp_color_wait");
        $("#m1_hum").removeClass("hum_color_wait");
        if(m1_command != 1000){
          //console.log('danger');
          reverse_counter = 60 - seconds_m1;
          document.getElementById("m1_button").innerHTML = 'Otkaži komandu ('+reverse_counter+' sek)';
        }
    }
    else if(minutes_m1>=1 && minutes_m1 <= 2){
        minutes_m1 = minutes_m1 - 1; 
        if(m1_command != 1000){  
          document.getElementById("m1_time").innerHTML = " <style>#m1_time{color:orange;}</style> Komanda se izvršava: " +minutes_m1+ " min " +seconds_m1+ " sek";
        }
        else{
          document.getElementById("m1_time").innerHTML = " <style>#m1_time{color:orange;}</style> Provjera komande na motoru: " +minutes_m1+ " min " +seconds_m1+ " sek";
        }
        $("#m1_temp").addClass("temp_color_wait");
        $("#m1_hum").addClass("hum_color_wait");
      if(seconds_m1 == 10){
      ...
      document.getElementById("m1_time").innerHTML = "<style>#m1_time{color:red; font-weight:600;}</style>  Nije na mreži:  " +minutes_m1 + "m " + seconds_m1 + "s";
  }
}
   //COUNTER FOR m1 END
    ...
    if(minutes_m2<1){
        ...
        if(m2_command != 1000){
          ...
          document.getElementById("m2_button").innerHTML = 'Otkaži komandu ('+reverse_counter+' sek)';
        }
    }
...
function m1Click(){
    ...
    current_status_m1 = $('#status-m1').text();
    console.log('current_status_m1:'+current_status_m1);
    new_command_M1 = $('#m1').val();
    motor = 'm1';
    
    final_command = new_command_M1 - current_status_m1;
             if(current_status_m1 != new_command_M1){ 
                if(new_command_M1 > current_status_m1){
                    
                    console.log("Otvori za "+final_command);
                }
                else if(new_command_M1 < current_status_m1){
            ...
            $.ajax({
              type: "POST",
              url: 'set_motor.php?m='+motor+'&c='+final_command+'&s='+new_command_M1,
              success: function(data) {
                //console.log(data);
                getData();
              }
...
function m2Click(){
  ...
  current_status_m2 = $('#status-m2').text();
  console.log('current_status_m2:'+current_status_m2);
  new_command_M2 = $('#m2').val();
  console.log('new_command_M2:'+new_command_M2);
  motor = 'm2';
